                        ...
                        @if ($data)
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <form onsubmit="return confirm(Hapus Data?)" class='d-inline' action=" {{ url ('/adjusment/delete/' .$item->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            ...
                                            @if (Auth::user()->id == 1)
                                                ...
                                                {{-- <button type="button" class="btn btn-warning btn-sm" onclick="detail({{ $item->id }}, {{ $item->shop_id }}, {{ $item->conversion_id }}, {{ $item->qty }}, '{{ $item->notes }}', '{{ $item->status }}')">Edit</button> --}}
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <input type="text" placeholder="Tipe" class="form-control" name="type" required value="IN" readonly>
                    </div>
                    <div class="form-group">
                        <label for="">Toko</label>
                        <select name="shop_id" id="shop" class="form-control select2" required value="{{ old('shop_id') }}" >
                    ...
                    <label>Qty</label>
                    <div class="form-group">
                        <input type="number" placeholder="Kuantitas" class="form-control" name="qty" required value="{{ old('qty') }}">
                    </div>
                    <label>Notes</label>
                    <div class="form-group">
...
{{-- Modal Box Edit --}}
<div class="modal fade text-left" id="modal_edit" role="dialog"
        ...
        <div class="modal-content">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <label>Type</label>
                    <div class="form-group">
                        <input type="text" placeholder="Tipe" class="form-control" name="type-edit" required value="IN" readonly>
                    </div>
                    <div class="form-group">
                        <label for="">Toko</label>
                        <select name="shop_id-edit" id="shop-edit" class="form-control select2" required value="{{ old('shop_id') }}" >
                            <option value="">-- Pilih Nama Toko --</option>
                    ...
                    <div class="form-group">
                        <label for="">Barang</label>
                        <select name="conversion_id-edit" id="conversion_id-edit" class="form-control select2" required >
                            <option value="">-- Pilih Nama Conversion --</option>
                    ...
                    <label>Qty</label>
                    <div class="form-group">
                        <input type="number" placeholder="Kuantitas" class="form-control" name="qty-edit" id="qty-edit" required value="{{ old('qty') }}">
                    </div>
                    <label>Notes</label>
                    <div class="form-group">
                        <input type="text" placeholder="Catatan" class="form-control" name="notes-edit" id="notes-edit" required value="{{ old('notes') }}" required>
                    </div>
                    <label>Status</label>
                    <div class="form-group">
...
<script>
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $.each(data.data, function(index, product) {
                    ...
                    $('#conversion_id').append('<option value="' + id + '">' + label + '</option>');
                });
            }
        })
    ...
    async function detail(id) {
        ...
        // Deklarasi ulang select2 ke select option toko
        $("#shop-edit").select2({
            dropdownParent: $("#modal_edit")
        })
    
        // Deklarasi ulang select2 ke select option barang
        $("#conversion_id-edit").select2({
            dropdownParent: $("#modal_edit")
        })
    
        // Set value kuantiti dan catatan sesuai dengan detail data
        ...
        $(".btn-update").click(function(e){
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    ...
                    $('.btn-update').html('')
                    $('.btn-update').append('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>Loading...')
                },
                success:async function(data){
                    if (data.success) {
                        setTimeout(() => {
